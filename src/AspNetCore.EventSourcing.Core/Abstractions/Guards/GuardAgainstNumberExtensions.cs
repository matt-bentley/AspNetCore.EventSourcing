...
namespace AspNetCore.EventSourcing.Core.Abstractions.Guards
{
    ...
    {
        public static void LessThanOrEqualTo(this IGuardClause guardClause, decimal input, decimal minValue, string parameterName = "Value", string units = "", string? message = null)
        {
            if (input <= minValue)
            {
        ...
        public static void LessThan(this IGuardClause guardClause, decimal input, decimal minValue, string parameterName = "Value", string units = "", string? message = null)
        {
            if (input < minValue)
            {
                Error(message ?? $"'{parameterName}' must be greater than or equal to {minValue}{units}.");
            }
        }
